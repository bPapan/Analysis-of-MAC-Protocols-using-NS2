...
#for {set i 0} {$i < [expr $flow]} {incr i} {
#	set fileQ_($i) [open out_queue_$i.tr w]
#}
...
while {$i < [expr $num_row*$num_col]} {
	$ns_ at [expr ($i*0.5)] "$node_($i) setdest [expr $x_dim*rand()] [expr $y_dim*rand()] $velocity"
	incr i;
};
...
for {set i 0} {$i < $num_parallel_flow } {incr i} {
	set udp_node $i
	set null_node [expr ($num_col*$num_row)-$i-2];#CHNG
...
#set k1 [expr ($i/2)]
#set k2 [expr ($num_col*$num_row)-$k1-2]
...
#set fileQP [open outq_par.tr w]
#$ns_ trace-queue $udp_($k1) $null_($k1) $fileQP
...
#set fileQC [open outq_crs.tr w]
#set qmon [$ns_ monitor-queue $node_($k1) $node_($k2) $fileQC 0.1]
#[$ns_ link $node_($k1) $node_($k2)] queue-sample-timeout
